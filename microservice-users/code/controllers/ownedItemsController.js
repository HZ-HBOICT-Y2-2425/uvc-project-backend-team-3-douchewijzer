...
export async function listOwnedItems(req, res) {
  ...
  try {
    ...
    for (const user of users) {
      ...
      const userStatistics = await executeQuery(
        pool,
        "SELECT * FROM statistics WHERE userID = ?",
        [userID]
      );
      ...
      const validTemperatures = userStatistics
      ...
      filteredBadges.forEach((badge) => {
        ...
        case 2:
          createdAt =
              userStatistics.find((stat) => parseFloat(stat.temperature) <= 28)
                ?.created_at || new Date();
          break;
        case 3:
          createdAt =
              userStatistics.find((stat) => parseFloat(stat.temperature) <= 22)
                ?.created_at || new Date();
          break;
        case 4:
          createdAt =
              userStatistics.find((stat) => parseFloat(stat.temperature) <= 18)
                ?.created_at || new Date();
          break;
        case 5:
          createdAt =
              userStatistics.find((stat) => coins >= 1000)?.created_at ||
              new Date();
          break;
        case 6:
          createdAt =
              userStatistics.find((stat) => coins >= 3000)?.created_at ||
              new Date();
          break;
        case 7:
          createdAt =
              userStatistics.find((stat) => coins >= 10000)?.created_at ||
              new Date();
          break;
        case 8:
          createdAt =
              userStatistics.find((stat) => statisticsEntriesCount >= 1)
                ?.created_at || new Date();
          break;
        case 9:
          createdAt =
              userStatistics.find((stat) => statisticsEntriesCount >= 10)
                ?.created_at || new Date();
          break;
        case 10:
          createdAt =
              userStatistics.find((stat) => statisticsEntriesCount >= 100)
                ?.created_at || new Date();
          break;
        }
...
export async function updateOwnedItems(req, res) {
  ...
  try {
    const result = await executeQuery(
      pool,
      "UPDATE owned_items SET itemPrice = ? WHERE userID = ? AND badgeID = ?",
      [itemPrice, userID, badgeID]
    );
...
export async function addOwnedItem(req, res) {
  ...
  try {
    const result = await executeQuery(
      pool,
      "INSERT INTO owned_items (userID, badgeID, itemPrice) VALUES (?, ?, ?)",
      [userID, badgeID, itemPrice]
    );
